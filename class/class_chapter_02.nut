...
=======
          ...
          local w_dir = my_tile(way1_coords.a).get_way_dirs(wt_road)
          if ( !dir.is_twoway(w_dir) ) {
            local waya = my_tile(way1_coords.a).find_object(mo_way)
            waya.mark()
>>>>>>> local
          }
          w_dir = my_tile(way1_coords.a).get_way_dirs(wt_road)
          if ( !dir.is_twoway(w_dir) ) {
            local wayb = my_tile(way1_coords.b).find_object(mo_way)
            wayb.mark()
          }

          //check_way_last_tile
          local test_way = test_select_way(my_tile(way1_coords.a), my_tile(way1_coords.b), wt_road)
          if (test_way) {

            //r_way.c = check_way_last_tile
            unmark_waybuild(wt_road)
            ...
            label_bord(c_way_limit1.a, c_way_limit1.b, 0, true, "X")

            //Creea un cuadro label
            label_bord(city1_limit1.a, city1_limit1.b, 0, false, "X")
            label_bord(city2_limit1.a, city2_limit1.b, 0, false, "X")
